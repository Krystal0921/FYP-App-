...
const ScreenQuiz = ({ route, navigation }) => {
  ...
  //         "http://44.221.91.193:3000/Lesson/Section/publicQuiz",
  //         {
  //           method: "POST",
  //           headers: {
  ...
  //       const response = await fetch(
  //         "http://44.221.91.193:3000/Lesson/Section/Quiz",
  //         {
  //           method: "POST",
  //           headers: {
  //             "Content-Type": "application/json",
  ...
  const onClickNext = async (index, correctAns) => {
    ...
    if (checked === correctAns) {
      ...
      Alert.alert(`The correct answer is ${correctAns}`);
    }
    ...
    if (nextIndex < read.length) {
      try {
        lessonQuiz.scrollToIndex({ animated: true, index: nextIndex });
      } catch (e) {
        console.error(e);
      }
    } else {
